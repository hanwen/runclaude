...
- **Transcript branch**: `refs/runclaude/<sid>/transcript` — linear, parentless root, single file `transcript.jsonl`, blob = full session file. One commit per coalesced batch (same-second events smushed); commit message lists covered msg-uuids **and the file offset/blob hash of the tip**, making flushes idempotent after a crash (restart compares tip metadata, not the state file).
...
- **jj support is no longer deferred**: `resolveRecordGitDir` falls back to the jj git backend store, following the `.jj/repo` pointer file of linked jj workspaces — colocated repos, pure-jj repos, and `jj workspace add` dirs all record and share the clone's sessions (keyed to the main workspace root). Snapshots always exclude `.jj/`. Pure-jj remains partially degraded: git HEAD is unborn, so checkpoints are rootless and bundles self-contained (no jj-aware base rule yet).
- **Checkpoints are native jj changes** (when host `jj` is available): per checkpoint the recorder resolves `@`'s change id with `jj log --ignore-working-copy` — creating no operation and immune to stale workspaces, so recording stays invisible to `jj op log`/`jj undo` and keeps working when another workspace rewrites an ancestor. Recorder-authored checkpoint commits embed the id in a `change-id` header (jj round-trips it on import, so a materialized checkpoint is a version of the author's change, with identical ids in every clone); when jj's own snapshot already matches the worktree — the last mutation came through a jj command — the checkpoint ref reuses the jj-authored working-copy commit verbatim, sha-identical. `jj util snapshot` is deliberately *not* used: it refuses in stale workspaces (no bypass; `snapshot.auto-update-stale` does not apply to it), and updating staleness from a background recorder would reset the working tree mid-session.
- **Subcommands over flags**: sessions are managed by first-argument verbs — `runclaude new` (start a recorded claude session), `resume [<sid>]` (recorded resume; defaults to the latest recorded session, auto-fork still applies), `list`, `upload <dest>`, `download <src>`, `rm <sid>...`. The flag spellings from the plan above (`--record`, `--sessions`, `--upload`, `--download`, `--record-rm`) were removed; recording is enabled by `new`/`resume`, by `record: true` in `.claude/runclaude.json`, or per session by stickiness.
- **Checkpoints amend, not stack**: the parent-chain scheme above (first checkpoint on session-start HEAD, each subsequent on the previous, moved HEADs as second parents) produced a growing commit stack that looked like history without being any. Now every checkpoint commit has the same parents and message as the state it amends — jj: `@`'s parents, description (so `jj desc` text appears on checkpoints), and change id; plain git: the current HEAD and an empty message — and only the tree differs; the per-uuid refs are the history. The `session:`/`uuids:`/`span:` message trailer is gone: nothing consumed it (the refs carry sid/timestamp/uuid, the meta branch carries the authoritative offsets), and it kept the commits from reading as versions of the user's change. Bundle math walks parents through checkpoint commits (`checkpointBases`), since after a restore the working copy sits on an earlier checkpoint.
- **Resume restores the tree; download only fetches**: `runclaude resume <sid>` materializes the session's latest checkpoint into the current worktree before launching claude (`record.RestoreCheckpoint`), so a session travels as conversation + tree together — including across workspaces of the same clone. In a jj workspace the checkpoint is made visible through a temporary local branch + `jj git import` (jj cannot resolve commits reachable only from `refs/runclaude/`), then `jj new <checkpoint>` puts `@` on top of it — the change-id header makes it a version of the recorded change — and the branch is removed via `jj bookmark delete` + `jj git export`, never by deleting the git ref directly (jj treats an externally deleted branch as rewritten history and would abandon the checkpoint, rebasing `@` off it). Plain git restores by detached checkout and only onto a clean worktree (jj needs no such guard: `jj new` snapshots the previous state first). A worktree already at the checkpoint tree (same-directory resume; compared via a throwaway-index snapshot with the recorder's exclusions) is left untouched, and an unrecorded sid is claude's to resolve. `download` accordingly shed `--at`/`--dest` and the worktree checkout: it just fetches the refs, and recorder startup materializes the transcript on the next resume.
