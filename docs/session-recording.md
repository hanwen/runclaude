...
- **Subcommands over flags**: sessions are managed by first-argument verbs — `runclaude new` (start a recorded claude session), `resume [<sid>]` (recorded resume; defaults to the latest recorded session, auto-fork still applies), `list`, `upload <dest>`, `download <src>`, `rm <sid>...` — with the original flag spellings (`--sessions`, `--upload`, `--download`, `--record-rm`) kept as aliases.
